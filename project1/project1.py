...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    #print(feature_vector, theta, thetaT, len(thetaT))
    #print(feature_vector.shape, theta.shape, thetaT.shape )
    matmul = np.dot(theta, feature_vector)
    pred  = matmul+theta_0
    #z = np.dot(label, pred)
    z = label*pred
    #print(matmul, pred,z)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for feature, label in zip(feature_matrix, labels):
        cnt += 1
        hinge_loss += hinge_loss_single(feature, label, theta, theta_0)

    return hinge_loss/cnt
...
def perceptron_single_step_update(
    ...
    #label = np.array(label)
    # thetaT = np.transpose(theta)
    #print(feature_vector, theta, theta_0, label)
    #print(feature_vector.shape, theta.shape, label.shape)
    matmul = np.dot(theta, feature_vector)
    pred = matmul + theta_0
    #z = np.dot(label, pred)
    z = label * pred
    ...
    #hinge_loss = 0
    if z <= 0:
        theta = theta + feature_vector*label
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    r,f = np.shape(feature_matrix)
    theta = np.zeros(f)
    theta_0 = 0
    lst = get_order(r)
    #print(lst)
    for i in range(T):
        for j in get_order(r):
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for i in range(T):
        ...
        for j in get_order(r):
            theta, theta_0 = perceptron_single_step_update(feature_matrix[j], labels[j], theta, theta_0)
            th_sum = th_sum + theta
            th0_sum = th0_sum + theta_0
            #theta = th
            #theta_0 = th_0
        #th_sum = 0.
        #th0_sum = 0.
    return th_sum/(r*T), th0_sum/(r*T)
...
def pegasos_single_step_update(
    ...
    #theta = np.array(theta)
    #feature_vector = np.array(feature_vector)
    #matmul = np.dot(feature_vector, theta)
    #pred = matmul + theta_0
    # z = np.dot(label, pred)
...
def classifier_accuracy(
    ...
    train_cls = classify(train_feature_matrix, theta, theta_0)
    train_acc = accuracy(train_cls, train_labels)

    #val_theta, val_theta_0 = classifier(val_feature_matrix, val_labels, **kwargs)
    val_cls = classify(val_feature_matrix, theta, theta_0)
    val_acc = accuracy(val_cls, val_labels)
    #print(val_cls, val_acc)
    #print(val_cls, val_acc)
